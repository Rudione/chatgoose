...
const Roast = {
    ...
    loadSettings() {
        ...
        const keysSaved = this._loadKeys();
    ...
    saveSettings() {
        ...
        this._saveKeys(keys);
        if (customUrl) {
            const v = Security.classifyEndpoint(customUrl);
            if (v.ok) localStorage.setItem('cg_roast_custom_url', customUrl);
            else localStorage.removeItem('cg_roast_custom_url');
        }
...
[{"category":"toxic","user":"NickName","quote":"исходный текст","reasoning":"короткое объяснение"}]`;
        ...
        if (p === 'anthropic') {
            ...
            const verdict = Security.classifyEndpoint(url);
            if (!verdict.ok) throw new Error(t('roastBadEndpoint') || 'Небезопасный адрес API. Разрешён только https на публичный домен.');
            if (p === 'custom' && key && !verdict.known) {
                const okKey = 'cg_roast_trusted_' + verdict.host;
                let trusted = false;
                try { trusted = localStorage.getItem(okKey) === '1'; } catch (e) {}
                if (!trusted) {
                    const warn = (t('roastCustomConfirm') || 'Ключ будет отправлен на сторонний сервер {h}. Продолжить?').replace('{h}', verdict.host);
                    if (!confirm(warn)) throw new Error(t('roastAborted') || 'Отменено');
                    try { localStorage.setItem(okKey, '1'); } catch (e) {}
                }
            }
    ...
    _renderRound() {
        ...
        if (quote) quote.innerHTML = `<span style="opacity:.85;">"${Emotes.parse(r.quote || '—')}"</span>${r.reasoning ? '<div style="margin-top:6px;font-size:11px;color:var(--c-muted);opacity:.75;font-style:normal;">💭 ' + Emotes.parse(r.reasoning) + '</div>' : ''}`;
        ...
        opts.forEach((name, i) => {
            ...
            b.innerHTML = `<span style="color:${color};font-weight:700;">${escHtml(name)}</span>`;
        ...
        if (list) {
            list.innerHTML = this.state.answers.map(a => {
                ...
                return `<div class="roast-result-card ${a.isOk ? 'ok' : 'fail'}">
                  ...
                  <div class="roast-result-who">→ <span style="color:${color}">${escHtml(a.correctUser)}</span></div>
                  ${a.quote ? `<div class="roast-result-quote">"${Emotes.parse(a.quote.slice(0,140))}"</div>` : ''}
                  ${a.reasoning ? `<div style="font-size:10px;color:var(--c-muted);">💭 ${Emotes.parse(a.reasoning)}</div>` : ''}
                </div>`;
            }).join('');
        }
