...
const Oracle = {
    ...
    onMessage(name, text, tags) {
        ...
        if (list) {
            ...
            item.innerHTML = `<span class="or-feed-name" style="color:${tags.color||'#9ca3af'}">${escHtml(name)}</span><span class="${cls}">${this.round.type === 'number' ? escHtml(String(display)) : Emotes.parse(display)}</span>`;
    ...
    _renderWinnerBox(winner, metaHtml) {
        ...
        if (winner) {
            wbox.innerHTML = `
                ...
                <div class="font-display" style="font-size:28px;font-weight:700;background:linear-gradient(135deg,${winner.color||'#fff'},var(--c-accent));-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent;">${escHtml(winner.name)}</div>
        ...
        if (ranked.length > 1) {
            ...
            ranked.slice(0, 10).forEach((e, i) => {
                ...
                row.innerHTML = `
                    ...
                    <div class="lc-podium-name" style="color:${e.color||'#9ca3af'}">${escHtml(e.name)}</div>
                    ${rightColsFn(e)}`;
                top.appendChild(row);
            });
        }
        ...
        if (lst) {
            ...
            if (!arr.length) {
                ...
                arr.slice(0, 15).forEach((u, i) => {
                    ...
                    row.innerHTML = `
                        <div class="lc-podium-rank ${rankCls}">${rank}</div>
                        <div class="lc-podium-name" style="color:${u.color||'#9ca3af'};font-size:13px;">${escHtml(u.name)}</div>
