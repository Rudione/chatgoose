...
const Raffle = {
    ...
    _updateSoundBtn() {
        ...
        if (!btn) return;
        btn.innerHTML = this.config.sound
            ...
            : '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><line x1="22" y1="9" x2="16" y2="15"/><line x1="16" y1="9" x2="22" y2="15"/></svg>';
        btn.style.opacity = this.config.sound ? '1' : '.55';
    },
    ...
    restore() {
        ...
        this._seenRowNames = new Set(this.entrants.keys());
    ...
    _BADGE_DEFS: {
        '🎥': { label: 'Стрим', color: '#e91916', glyph: '<path d="M23 7l-7 5 7 5V7z"/><rect x="1" y="5" width="15" height="14" rx="2"/>' },
        '🛡️': { label: 'Мод', color: '#00ad03', glyph: '<path d="M14.5 17.5L3 6V3h3l11.5 11.5"/><path d="M13 19l6-6"/><path d="M16 16l4 4"/><path d="M19 21l2-2"/>' },
        '💎': { label: 'VIP', color: '#e005b9', glyph: '<path d="M6 3h12l4 6-10 13L2 9z"/><line x1="2" y1="9" x2="22" y2="9"/>' },
        '⭐': { label: 'Саб', color: '#8b7dff', glyph: '<polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/>' },
        '👑': { label: 'Prime', color: '#0f9bd7', glyph: '<path d="M2 20h20"/><path d="M5 20L2 8l5 5 5-8 5 8 5-5-3 12H5z"/>' }
    },
    _bsq(def, size) {
        const s = size || 14;
        return `<span class="rf-bsq" style="--bc:${def.color};width:${s}px;height:${s}px;" title="${def.label}"><svg width="${s - 5}" height="${s - 5}" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round">${def.glyph}</svg></span>`;
    },
    _badgeIcons(badges, size) {
        if (!badges) return `<span class="rf-bsq rf-bsq-plain" style="--bc:#5a5a72;width:${size || 14}px;height:${size || 14}px;"><svg width="${(size || 14) - 5}" height="${(size || 14) - 5}" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></span>`;
        return Object.keys(this._BADGE_DEFS).filter(k => badges.includes(k)).map(k => this._bsq(this._BADGE_DEFS[k], size)).join('');
    },
    _badgeHtml(badges) {
        if (!badges) return '';
        return Object.keys(this._BADGE_DEFS).filter(k => badges.includes(k))
            .map(k => { const d = this._BADGE_DEFS[k]; return `<span class="rf-role-pill" style="background:${d.color}1a;border-color:${d.color}55;color:${d.color};">${this._bsq(d, 13)} ${d.label}</span>`; })
            .join('');
    },
    ...
    _rc(color) {
        ...
        if (this._rcCache[color]) return this._rcCache[color];
        let out = color;
    ...
    _TICKET_SVG: '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1.5px;"><path d="M2 9a3 3 0 0 1 0 6v2a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-2a3 3 0 0 1 0-6V7a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2Z"/><path d="M13 5v2"/><path d="M13 17v2"/><path d="M13 11v2"/></svg>',
    _X_SVG: '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round"><path d="M18 6 6 18M6 6l12 12"/></svg>',
    _CHECK_SVG: '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1.5px;"><path d="M20 6 9 17l-5-5"/></svg>',
    _PERSON_SVG: '<svg width="55%" height="55%" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.5-7 8-7s8 3 8 7"/></svg>',
    _rowHTML(name, e, isNew) {
        return `<div class="rf-entrant-row${isNew ? ' rf-row-new' : ''}" data-name="${escHtml(name)}">
                  <span class="rf-entrant-badges">${this._badgeIcons(e.badges, 14)}</span>
                  <span class="rf-entrant-name" style="color:${this._rc(e.color)}">${escHtml(name)}</span>
                  <span class="rf-entrant-tickets" data-v="${e.tickets}">${e.tickets} ${this._TICKET_SVG}</span>
                  <button class="rf-entrant-x" onclick="Raffle.removeEntrant('${name.replace(/'/g, "\\'")}')">${this._X_SVG}</button>
                </div>`;
    },
    _renderEntrants(full) {
        ...
        requestAnimationFrame(() => {
            ...
            if (full || q || !this._rowEls) {
                list.innerHTML = shown.map(([name, e]) => this._rowHTML(name, e, false)).join('');
                this._rowEls = null;
                if (!q) {
                    this._rowEls = new Map();
                    list.querySelectorAll('.rf-entrant-row').forEach(el => this._rowEls.set(el.dataset.name, el));
                }
                return;
            }
            const want = new Set(shown.map(([n]) => n));
            this._rowEls.forEach((el, n) => {
                if (!want.has(n)) { if (el.parentNode) el.remove(); this._rowEls.delete(n); }
            });
            let anchor = list.firstChild;
            shown.forEach(([name, e]) => {
                let el = this._rowEls.get(name);
                if (!el) {
                    const tpl = document.createElement('div');
                    tpl.innerHTML = this._rowHTML(name, e, true);
                    el = tpl.firstElementChild;
                    this._rowEls.set(name, el);
                    list.insertBefore(el, anchor);
                } else {
                    const tk = el.querySelector('.rf-entrant-tickets');
                    if (tk && tk.dataset.v != e.tickets) { tk.dataset.v = e.tickets; tk.innerHTML = e.tickets + ' ' + this._TICKET_SVG; }
    ...
    reroll() {
        ...
        if (sub) {
            if (this.winners.length) {
                ...
                sub.innerHTML = `<span style="color:${this._rc(w.color)};font-weight:700;">${escHtml(w.name)}</span> ${t('rfRerollReplace') || 'будет заменён новым победителем'}.<br><span style="font-size:11px;">${t('rfEntrants') || 'Участников'}: ${this.entrants.size}</span>`;
            } else {
    ...
    _avatarHtml(name, color, size) {
        const fb = `<span class="rf-ava-fb" style="width:${size}px;height:${size}px;background:${color}22;color:${color};">${this._PERSON_SVG}</span>`;
        if (!this.config.avatars) return fb;
        return `<span class="rf-ava-wrap" style="width:${size}px;height:${size}px;background:${color}22;color:${color};"><img class="rf-ava" loading="eager" style="width:${size}px;height:${size}px;" src="https://unavatar.io/twitch/${encodeURIComponent(name)}?fallback=false" onerror="this.remove()" alt="">${this._PERSON_SVG}</span>`;
    },

    _avatarHtmlFor(rec, size) {
        const color = this._rc(rec.color);
        if (!this.config.avatars) return `<span class="rf-ava-fb" style="width:${size}px;height:${size}px;background:${color}22;color:${color};">${this._PERSON_SVG}</span>`;
    ...
    _renderChatHistory(rec, opts) {
        opts = opts || {};
        const color = this._rc(rec.color);
        const row = (m, isNew) => `<div class="rf-chat-msg${isNew ? ' rf-chat-msg-new' : ''}"><span class="rf-chat-time">${this._fmtClock(m.at)}</span><span class="rf-chat-body"><span class="rf-chat-name" style="color:${color};">${escHtml(rec.name)}</span> ${Emotes.parse(m.text)}</span></div>`;
        let html = '';
    ...
    async _fetchWinnerInfo(rec) {
        ...
        if (subRes.status === 'fulfilled') {
            ...
            rec._sub = j || null;
            rec.followAge = j?.followedAt ? (this._CHECK_SVG + ' ' + this._fmtAgo(j.followedAt)) : (t('rfNotFollowing') || 'не фоллов');
                ...
                if (notFollowing || tooFresh) {
                    ...
                    if (note) {
                        ...
                        note.innerText = notFollowing
                            ? (t('rfWinnerNotFollower') || 'Победитель не фолловер — можно рерольнуть')
                            : (t('rfWinnerFreshFollow') || 'Фоллов моложе минимума — можно рерольнуть');
        ...
        box.innerHTML = this.winners.slice(0, 4).map((w, i) => {
                ...
                return `<div class="rf-winner-card rf-winner-top">
                    ...
                    <div class="rf-winner-main rf-winner-clickable" ${openAttr} title="${t('rfOpenProfile') || 'Открыть профиль'}">
                        ...
                        <div style="flex:1;min-width:0;">
                            <div class="rf-winner-name" style="color:${this._rc(w.color)};font-size:20px;">${escHtml(w.name)}</div>
            ...
            return `<div class="rf-winner-card rf-winner-past rf-winner-clickable" ${openAttr} title="${t('rfOpenProfile') || 'Открыть профиль'}">
                ...
                <div style="flex:1;min-width:0;">
                    ...
                    <div class="rf-winner-name" style="color:${this._rc(w.color)};font-size:12px;">${escHtml(w.name)}</div>
                </div>
                ${msgCount}
            ...
            if (lst) {
                lst.innerHTML = this.history.length ? this.history.map((w, i) => `
                    <div class="rf-hist-row">
                      ...
                      <span class="rf-entrant-name" style="color:${this._rc(w.color)};flex:1;">${this._badgeIcons(w.badges, 13)} ${escHtml(w.name)}</span>
    ...
    _renderSummary() {
        ...
        const roleLabels = { sub: t('rfRoleSub') || 'Саб', mod: t('rfRoleMod') || 'Модер', vip: t('rfRoleVip') || 'VIP', firstTimer: t('rfRoleFirst') || 'Новичок' };
    ...
    _renderWinnerCard(rec) {
        ...
        body.innerHTML = `
            <div class="rf-wc-head">
                ...
                <div style="flex:1;min-width:0;">
                    <a href="${link}" target="_blank" rel="noopener noreferrer" class="rf-wc-name-link" title="${t('rfWcGoChannel') || 'Перейти на канал'}">
                        <span class="rf-wc-name" style="color:${this._rc(rec.color)};">${escHtml(u.displayName || rec.name)}</span>
                ...
                <button class="rf-iconbtn rf-wc-close" onclick="Raffle.closeWinnerCard()">${this._X_SVG}</button>
            </div>
            <div class="rf-wc-stats">
                ...
                ${stat(rec.tickets + ' ' + this._TICKET_SVG, t('rfWcTickets') || 'Билетов', 'var(--c-gold)')}
            </div>
            ${followBlock}
            ${subBlock}
    ...
    _renderConnLog(log) {
        ...
        const dotColor = type => type === 'ok' ? '#52ffb6' : type === 'warn' ? '#ff6b91' : '#ffd470';
        const ico = type => `<svg width="8" height="8" viewBox="0 0 8 8"><circle cx="4" cy="4" r="4" fill="${dotColor(type)}"/></svg>`;
