...
function getSpotifyInfo(url){const m=url.match(/open\.spotify\.com\/(intl-[a-z]+\/)?(track|album|playlist|episode)\/([A-Za-z0-9]+)/);if(m)return{type:m[2],id:m[3]};const m2=url.match(/spotify\.com\/(track|album|playlist|episode)\/([A-Za-z0-9]+)/);if(m2)return{type:m2[1],id:m2[2]};return null;}
